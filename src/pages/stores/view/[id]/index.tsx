...
import { useStoreFindFirst } from 'lib/roq';
...
function StoreViewPage() {
  ...
  const queryParams = useMemo<{ include: { supplier: boolean } }>(
    () =>
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem
                  label="Opening Hours"
                  text={data?.opening_hours ? format(data?.opening_hours, 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Closing Hours"
                  text={data?.closing_hours ? format(data?.closing_hours, 'dd-MM-yyyy') : ''}
                />
